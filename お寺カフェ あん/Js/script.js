document.addEventListener('DOMContentLoaded', () => {
    ...
    if (spFixedCta && footer) {
        const ctaObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    spFixedCta.classList.add('translate-y-full', 'opacity-0', 'pointer-events-none');
                } else {
                    spFixedCta.classList.remove('translate-y-full', 'opacity-0', 'pointer-events-none');
                }
            });
        }, { threshold: 0.1 });
