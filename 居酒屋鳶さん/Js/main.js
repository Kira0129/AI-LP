...
(function () {
  ...
  function buildSP() {
    ...
    spWrap.style.cssText = `width:100%;height:${slideH}px;overflow:hidden;position:relative;z-index:8;flex-shrink:0;background:var(--cream);`;
...
// Voice Read More
(function () {
  const btns = document.querySelectorAll('.voice-read-more');
  btns.forEach(btn => {
    btn.addEventListener('click', function() {
      const textWrap = this.previousElementSibling;
      const text = textWrap ? textWrap.querySelector('.voice-text') : null;
      if (!text) return;
...
document.addEventListener('dragstart', e => { if (e.target.tagName === 'IMG') e.preventDefault(); }, { passive: false });
