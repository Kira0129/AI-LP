...
(function () {
  ...
  function styleCard(card, role, pos, animate) {
    ...
    if (role === 'center') {
      ...
      card.classList.add('pos-center'); card.classList.remove('pos-topright', 'pos-botleft', 'pos-hidden');
  ...
  function buildSP() {
    ...
    spWrap.style.cssText = `width:100%;height:${slideH}px;overflow:hidden;position:relative;z-index:8;flex-shrink:0;background:var(--dark-brown);`;
    ...
    spWrap.addEventListener('touchmove', e => {
      if (locked === null) { const dx = Math.abs(e.touches[0].clientX - sx); const dy = Math.abs(e.touches[0].clientY - sy); locked = dx > dy ? 'h' : 'v'; }
...
(function () {
  ...
  function openMenu() { btn.classList.add('is-open'); overlay.classList.add('is-open'); btn.setAttribute('aria-expanded', 'true'); document.body.style.overflow = 'hidden'; }
  function closeMenu() { btn.classList.remove('is-open'); overlay.classList.remove('is-open'); btn.setAttribute('aria-expanded', 'false'); document.body.style.overflow = ''; }
  btn.addEventListener('click', () => { btn.classList.contains('is-open') ? closeMenu() : openMenu(); });
  overlay.querySelectorAll('a').forEach(a => { a.addEventListener('click', closeMenu); });
})();

// Prevent Image Dragging
document.addEventListener('contextmenu', function(e) { if(e.target.tagName === 'IMG') e.preventDefault(); }, { passive: false });
document.addEventListener('dragstart', function(e) { if (e.target.tagName === 'IMG') e.preventDefault(); }, { passive: false });
